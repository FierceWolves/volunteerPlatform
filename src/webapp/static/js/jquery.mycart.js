	...
	var loadMyCartEvent = function(userOptions) {
		...
		if(!$("#" + idCartModal).length) {
			$('body').append(
				...
				'<button type="button" class="btn btn-primary '  + classCheckoutCart + '">确认</button>'+
		...
		var drawTable = function() {
			...
			$cartTable.append(products.length ?
				'<tr>' +
				'<td></td>' +
				'<td><strong>共计</strong></td>' +
				'<td></td>' +
				'<td></td>' +
				'<td><strong id="' + idGrandTotal + '"></strong></td>' +
				'<td></td>' +
				'</tr>' :
				'<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">购物车里什么东西都没有，快去添加点商品吧。</div>'
			);
